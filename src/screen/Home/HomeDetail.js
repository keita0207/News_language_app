import React,{useState, Component,useLayoutEffect,useEffect} from 'react';
import { View, Text, Dimensions, SafeAreaView, ScrollView,
         Image, TouchableHighlight, StyleSheet,Button,FlatList,TouchableOpacity }
        from 'react-native';
import FontAwesome from 'react-native-vector-icons/FontAwesome'
import { connect } from 'react-redux'
import { bindActionCreators } from 'redux'
import { fetchUser } from '../../redux/actions/index'
import firebase from 'firebase'
require('firebase/firestore')



function PersonProfile (){

    const [isPress, setIsPress] = useState(false)
    const [follow,setFollowing] = useState(false)
    const touchProps = {
        activeOpacity : 1,
        underlayColor: '#c97fc7',
        color:'white',
        style: isPress ? styles.btnpress : styles.btnnormal,
        onHideUnderlay : () => setIsPress(false),
        onShowUnderlay : () => setIsPress(true),
        onPress: () => console.log('Changed.')
    }

    useLayoutEffect(() =>{
        navigation.setOptions({
          headerTitle: 'Profile',
          headerRight:() =>(
              <TouchableOpacity style={{marginRight:20}} onPress={() => navigation.navigate('EditProfile')}>
                  <FontAwesome name="edit" size={25} color="white" />
              </TouchableOpacity>
          )
        })
      },[navigation])




      const [user,setUser] = useState(null)

      



      const { width } = Dimensions.get('window') // windowを指定すると、フル画面の要素が取れる。
      const height = width * 100 / 100

     


        if(user === null){
            return(
                <View />
            )
        }
         return (
            <SafeAreaView style={{marginBottom:100}}>
                  <View style={{height:height,justifyContent:'center',alignItems:'center',marginTop:0,width:'100%'}}>
                      <ScrollView pagingEnabled horizontal style={{height,width,flexDirection:'row'}}>

                        {/* Needs to be changed to more efficient way */}
                        
                                <View>
                                    <Image style={{height:430,width:414}} source={{uri:'data:image/png;base64,'}} />
                                </View>
                        
                                <View pagingEnabled={false} />
                      
                          {/*FlatList
                          horizontal={true}
                          data={posts}
                          renderItem={({item}) =>(
                              <View>
                                  {item === undefined ? (
                                      <Image source={require('../../assets/images/user.jpg')} />
                                  ):
                                  (
                                      <Image
                                      style={{width,height,resizeMode:'contain'}}
                                      source={{uri:item.downloadURL}} />
                                  )}
                              </View>
                                  )} />*/}
                      </ScrollView>
                  </View>
                  <View style={{height:50,borderBottomColor:'black',borderBottomWidth:1}}>
                      <View style={{marginTop:15,flexDirection:'row',alignItems:'center',
                                  marginRight:5,marginHorizontal:20}}>
                          <Text style={{fontSize:25}}> / </Text>
                          <Text style={{fontSize:25}}> /</Text>
                      </View>
                  </View>
                              <ScrollView
                              pagingEnabled horizontal
                                  style={{
                                  marginTop:15,alignContent:'stretch',flexDirection:'row',width:'100%',height:40,
                                            marginRight:0,marginHorizontal:0}}>
                                
                              </ScrollView>
                      <ScrollView style={{height: Dimensions.get('screen').height}}>
                        <View style={{marginHorizontal:10,marginTop:30}}>
                              <Text style={{fontSize:20}}></Text>
                        </View>
                       {/*  <TouchableOpacity
                        onPress={() => props.navigation.navigate('EditProfile')}
                        style={{position:'absolute',top:0,right:10,justifyContent:'center',
                                                  alignItems:'center',borderRadius:10,padding:7,backgroundColor:'blue'}}>
                              <FontAwesome name="edit" style={{fontSize:40,color:'white'}} />
                        </TouchableOpacity>*/}


                                       <TouchableOpacity
                                             onPressIn={() => functionCombined(user)}
                                        // onPress={() => props.navigation.navigate('EnterChat',{user})}
                                            style={{position:'sticky',justifyContent:'center',alignItems:'center',
                                            paddingVertical:20,width:'100%'}}>
                                                <View style={{backgroundColor:'blue',width:'70%',paddingVertical:10,
                                                            justifyContent:'center',alignItems:'center'}}>
                                                <Text style={{color:'white'}}>Send a Message</Text>
                                                </View>
                                       </TouchableOpacity>
                                    ):
                                    (
                                        <View />
                                    )
                                }
                          
                    </ScrollView>
            </SafeAreaView>
          );
  };


const styles =  StyleSheet.create({
    btnpress:{
        justifyContent:'center',
        height:30,
        color:'white'
    },
    btnnormal:{
        backgroundColor:'aliceblue',
        justifyContent:'center',
        height:30,
    },
    languageText:{
        backgroundColor:'gray',
        marginLeft:0,

    }
})



export default PersonProfile;


/*

import React,{useState, Component,useLayoutEffect,useEffect} from 'react';
import { View, Text, Dimensions, SafeAreaView, ScrollView,
         Image, TouchableHighlight, StyleSheet,Button,FlatList,TouchableOpacity }
        from 'react-native';
import FontAwesome from 'react-native-vector-icons/FontAwesome'
import { connect } from 'react-redux'
import firebase from 'firebase'
require('firebase/firestore')
import EditProfile from './EditProfile'



function PersonProfile (props,{navigation}){

    const [isPress, setIsPress] = useState(false)
    const touchProps = {
        activeOpacity : 1,
        underlayColor: '#c97fc7',
        color:'white',
        style: isPress ? styles.btnpress : styles.btnnormal,
        onHideUnderlay : () => setIsPress(false),
        onShowUnderlay : () => setIsPress(true),
        onPress: () => console.log('Changed.')
    }

    /*useLayoutEffect(() =>{
        navigation.setOptions({
          headerTitle: 'Profile',
          headerRight:() =>(
              <TouchableOpacity style={{marginRight:20}} onPress={() => props.navigation.navigate('EditProfile')}>
                  <FontAwesome name="edit" size={25} color="white" />
              </TouchableOpacity>
          )
        })
      },[navigation])



    
      const { currentUser,posts } = props

      useEffect(() =>{
          if(props.route.params.uid === firebase.auth().currentUser.uid){
              setUser(currentUser)
              setUserPost(posts)
          }
          else{
              firebase.firestore()
              .collection('users')
              .doc(props.route.params.uid)
              .get()
              .then((snapshot) =>{
                  if(snapshot.exists){
                      setUser(snapshot.data())
                  }
                  else{
                      console.log('Does not exist.')
                  }
              })
              firebase.firestore()
              .collection('posts')
              .doc(firebase.auth().currentUser.uid)
              .collection('userPosts')
              .get()
              .then((snapshot) =>{
                  let posts = snapshot.docs.map(doc =>{
                      const data = doc.data()
                      const id = doc.id
                      return{id, ...data}
                  })
                  setUserPost(posts)
              })
          },[firebase.auth().currentUser.uid])
  
      const { width } = Dimensions.get('window') // windowを指定すると、フル画面の要素が取れる。
      const height = width * 100 / 100
  
  
         return (
            <SafeAreaView>
                  <View style={{height:height,justifyContent:'center',alignItems:'center',marginTop:0,width:'100%'}}>
                      <View pagingEnabled horizontal style={{height,width}}>
                          <FlatList
                          horizontal={true}
                          data={posts}
                          renderItem={({item}) =>(
                              <View>
                                  {item === undefined ? (
                                      <Image source={require('../../assets/images/user.jpg')} />
                                  ):
                                  (
                                      <Image
                                      style={{width,height,resizeMode:'contain'}}
                                      source={{uri:item.downloadURL}} />
                                  )}
                              </View>
                          )} />
                      </View>
                  </View>
                  <View style={{height:50,borderBottomColor:'black',borderBottomWidth:1}}>
                      <View style={{marginTop:15,flexDirection:'row',alignItems:'center',
                                  marginRight:5,marginHorizontal:20}}>
                          <Text style={{fontSize:25}}>{currentUser.name} / </Text>
                          <Text style={{fontSize:25}}>{currentUser.age} /</Text>
                      </View>
                  </View>
                              <View style={{marginTop:15,flexDirection:'row',alignItems:'center',width:'100%',
                                            marginRight:5,marginHorizontal:10}}>
                              {
                                  currentUser.selectedItems.map((item) =>(
                                      <View key={item.toString()}>
                                          <Text
                                              style={{marginLeft:10,borderColor:'#363434',borderWidth:2,padding:5}}>
                                            {item}
                                          </Text>
                                      </View>
                                  ))
                              }
                              </View>
                      <ScrollView style={{height: Dimensions.get('screen').height}}>
                        <View style={{marginHorizontal:10,marginTop:60}}>
                              <Text style={{fontSize:20}}>{currentUser.introduction}</Text>
                        </View>
                        <TouchableOpacity
                        onPress={() => navigation.navigate('EditProfile')}
                        style={{position:'absolute',top:0,right:10,justifyContent:'center',
                                                  alignItems:'center',borderRadius:10,padding:7,backgroundColor:'blue'}}>
                              <FontAwesome name="edit" style={{fontSize:40,color:'white'}} />
                        </TouchableOpacity>
                      </ScrollView>
            </SafeAreaView>
          );
  };
  
  const styles =  StyleSheet.create({
      btnpress:{
          justifyContent:'center',
          height:30,
          color:'white'
      },
      btnnormal:{
          backgroundColor:'aliceblue',
          justifyContent:'center',
          height:30,
      },
      languageText:{
          backgroundColor:'gray',
          marginLeft:0,
  
      }
  })
  
  
  const mapStateToProps = (store) =>({
      currentUser:store.userState.currentUser,
      posts: store.userState.posts
  })
  export default connect(mapStateToProps,null)(PersonProfile);

*/